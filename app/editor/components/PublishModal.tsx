...
export default function PublishModal({ code, initialPrompt, onClose }: PublishModalProps) {
  ...
  const [formData, setFormData] = useState({
    title: initialPrompt.substring(0, 20) || 'Untitled AI Game',
    author: 'Anonymous Creator',
    description: `AI Prompt: ${initialPrompt}`,
    genre: 'Action'
  });
  ...
  const handleStartAiThumbnail = () => {
    ...
    const timer = setInterval(() => {
      setCountdown((prev) => {
        if (prev <= 1) {
          clearInterval(timer);
          // 実際にはここでGemini API経由でImagen等を叩く
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-950/98 backdrop-blur-2xl p-6 md:p-12 overflow-y-auto animate-in fade-in duration-300">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="flex justify-between items-center mb-12">
          <button onClick={onClose} className="text-[10px] font-black text-slate-500 hover:text-white transition-colors uppercase tracking-[0.3em]">
            ← Back to Editor
          </button>
          <div className="text-[10px] font-black text-blue-500 italic uppercase tracking-[0.4em]">
            Step 02: Finalizing Metadata
          </div>
        </div>
